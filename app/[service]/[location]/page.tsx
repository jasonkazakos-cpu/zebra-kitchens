...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!svc || !loc) return {};
  const title = `${svc.title} in ${loc.name} | Zebra Kitchens`;
  const description = `Professional ${svc.title.toLowerCase()} services in ${loc.name}, ${loc.county}. ${svc.tagline ?? svc.teaser} Free no-obligation quotes from Zebra Kitchens.`;
  return {
    title,
    description,
    ...
    openGraph: {
      ...
      ...(loc.featuredImage && {
        images: [{ url: loc.featuredImage, width: 1200, height: 630, alt: `${svc.title} in ${loc.name} — Zebra Kitchens` }],
      }),
    },
    twitter: {
...
export default async function ServiceLocationPage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    url: `https://www.zebrakitchens.co.uk/${serviceParam}/${locationParam}`,
    hasOfferCatalog: {
      "@type": "OfferCatalog",
      name: svc.title,
  ...
  const faqs = [
    {
      ...
      answer: `Yes — Zebra Kitchens carries out ${svc.title.toLowerCase()} work in ${loc.name} regularly. ${loc.intro.split(".")[0]}.`,
    },
    {
      ...
      <section className="bg-chalk">
        <Container className="py-16 sm:py-20">
          <SectionHeading
            ...
            title={`Why Choose Zebra Kitchens for ${svc.title} in ${loc.name}`}
      ...
      <section className="bg-paper">
        <Container className="py-12 sm:py-16">
          <div className="flex flex-wrap gap-4 text-sm">
            <Link href={svc.href ?? "/kitchens"} className="font-medium text-accent-dark hover:underline">
